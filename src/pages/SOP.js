...
function SOP() {
    ...
    const [tempModelNumbers, setTempModelNumbers] = useState(Array.from({ length: 12 }, (_, index) => index + 1)); //建立12格3D Model List
    ...
    const [selectTempModelErrors, setSelectTempModelErrors] = useState({ //選擇暫存 3D Model的錯誤訊息
        tempModelImage: "",
        tempModelFile: ""
    });
    ...
    const handleSaveSOP = async (e) => {
        ...
        for (var i = 0; i < sops.length; i++) {
            ...
            formData.append(`sops[${i}].isDeletedSOPImage`, sops[i].isDeletedSOPImage);
            formData.append(`sops[${i}].sopVideo`, sops[i].sopVideo);
            formData.append(`sops[${i}].sopVideoObj`, sops[i].sopVideoObj);
            formData.append(`sops[${i}].isDeletedSOPVideo`, sops[i].isDeletedSOPVideo);
            ...
            for (var j = 0; j < sops[i].sopModels.length; j++) {
                formData.append(`sops[${i}].sopModels[${j}].sopModelId`, sops[i].sopModels[j].sopModelId);
                formData.append(`sops[${i}].sopModels[${j}].deleted`, sops[i].sopModels[j].deleted);
                formData.append(`sops[${i}].sopModels[${j}].sopId`, sops[i].sopModels[j].sopId);
                formData.append(`sops[${i}].sopModels[${j}].sopModelImage`, sops[i].sopModels[j].sopModelImage);
                formData.append(`sops[${i}].sopModels[${j}].sopModelImageObj`, sops[i].sopModels[j].sopModelImageObj);
                formData.append(`sops[${i}].sopModels[${j}].sopModelFile`, sops[i].sopModels[j].sopModelFile);
                formData.append(`sops[${i}].sopModels[${j}].sopModelFileObj`, sops[i].sopModels[j].sopModelFileObj);
            }
        }
    ...
    const handleSaveTempModel = async (e) => {
        ...
        //判斷圖片/檔案是否為空
        if (selectTempModel && (selectTempModel.tempModelImageObj == null || selectTempModel.tempModelFileObj == null)) {
            let newSelectTempModelErrors = { ...selectTempModelErrors };
            ...
            if (selectTempModelErrors.tempModelImage == "" && selectTempModelErrors.tempModelFile == "") {
    ...
    const onModelDragEnd = (event) => {
        ...
        if (source.droppableId == "3DModel" && destination.droppableId == "3DModel") {
            return;
        }
    ...
    return (
        <>
            <section className="content-header">
                <div className="container-fluid">
                    <div className="d-flex mb-2 justify-content-between align-items-center">
                        <div>
                            <a href={`/machine/${machineId}/machineAlarm`} className="d-flex align-items-center">
                                <i className="fas fa-angle-left"></i>&nbsp;&nbsp;{t("machineAlarm.content.header")}{/*Alarm管理*/}
                            </a>
                        </div>
                        <div className="content-header-text-color">
                            <h1><strong>{t("sop.content.header")}{/*SOP*/}</strong></h1>
                        </div>
                        <div>
                            <button
                                type="button"
                                    ...
                                    saveSOPLoading ?
                                        <>
                                            <Spinner
                                                ...
                                                aria-hidden="true"
                                            />
                                        </> : <><i className="fas fa-plus"></i> {t("machineIOT.btn.save")}{/*儲存設定*/}</>
                                }
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="row">
                        <div className="col-sm-12 col-md-2 mb-2">
                            <div className="sop-col-border-l">
                                            ...
                                            >
                                                {
                                                    sops.map((item, index) => {
                                                        if (item.deleted == 0) {
                                                            return (
                                                                <Draggable key={index} draggableId={item.sopStep.toString()} index={index}>
                                                                    {provided => (
                                                                        <div
                                ...
                                <div className="card" style={{ cursor: 'pointer', marginBottom: 0 }} onClick={() => handleAddSOP()}>
                            ...
                            <div className="sop-col-border-r">
                                {
                                    selectSOP != null ?
                                                ...
                                                <div className="row mb-3">
                                                    <div className="col-12">
                                                        <div className="form-group">
                                                            <label>{t("sop.sopMessage")}{/*步驟資訊*/}</label>
                                                            <textarea
                                                ...
                                                <div className="row mb-3">
                                                    <div className="col-sm-12 col-md-6">
                                                        <div className="form-group" style={{ minHeight: '300px' }}>
                                                            <label>{t("sop.sopVideo")}{/*影片*/}</label>
                                                            <div className="d-flex align-items-center justify-content-center sop-file-view">
                                                                {
                                                                    selectSOP.sopVideo != "" || selectSOP.sopVideoObj != null ?
                                                                        <video
                                                                            ref={videoSrcRef}
                                                                            width="100%"
                                                                            controls
                                                                            src={selectSOP.sopVideoObj != null ? URL.createObjectURL(selectSOP.sopVideoObj) : selectSOP.sopVideo}
                                                                        >
                                                                        </video>
                                                                        :
                                                                        <></>
                                                        ...
                                                        <div className="mt-3">
                                                            <button
                                                                ...
                                                                onClick={(e) => handleUploadVideoBtn(e)}>{t("sop.btn.uploadSopVideo")}{/*上傳影片*/}</button>
                                                            <input
                                                                type="file"
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={(e) => handleRemoveVideoBtn(e)}>{t("sop.btn.removeSopVideo")}{/*移除影片*/}</button>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-12 col-md-6">
                                                        <div className="form-group" style={{ minHeight: '300px' }}>
                                                            <label>{t("sop.sopImage")}{/*圖片*/}</label>
                                                            <div className="d-flex align-items-center justify-content-center sop-file-view">
                                                                {
                                                                    selectSOP.sopImage != "" || selectSOP.sopImageObj != null ?
                                                                        <img
                                                                            alt="not found"
                                                                            src={selectSOP.sopImageObj != null ? URL.createObjectURL(selectSOP.sopImageObj) : selectSOP.sopImage}
                                                                            style={{ width: '100%' }}
                                                                        />
                                                                        :
                                                                        <></>
                                                        ...
                                                        <div className="mt-3">
                                                            ...
                                                            >{t("sop.btn.uploadSopImage")}{/*上傳圖片*/}</button>
                                                            <input
                                                                type="file"
                                                                className="form-control d-none"
                                                                name="sopImage"
                                                            ...
                                                            <button
                                                                className="btn btn-danger"
                                                                onClick={(e) => handleRemoveImageBtn(e)}>{t("sop.btn.removeSopImage")}{/*移除圖片*/}</button>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="row">
                                                                        ...
                                                                        >
                                                                            {
                                                                                tempModelNumbers.map((item, index) => {
                                                                                    return (
                                                                                        <Draggable key={index} draggableId={`3DModelList-index-${index}`} index={index}>
                                                                                            {provided => (
                                                                                                <div
                                                                                                    ...
                                                                                                    ref={provided.innerRef}
                                                                                                >
                                                                                                    <div className="sop-model-col" onClick={(e) => handleOpenSaveTempModelBtn(e, index)}>
                                                                                                        {tempModels[index].tempModelImageObj != null ? <img src={URL.createObjectURL(tempModels[index].tempModelImageObj)} style={{ width: "100%", height: "100%" }} /> : <></>}
                                                                                                        {tempModels[index].tempModelImageObj != null ?
                                                                                                            <div className="remove-temp-model" style={{ position: "absolute", top: "0px", right: "7px" }}>
                                                            ...
                                                            <div className="form-group">
                                                                ...
                                                                <Droppable droppableId="3DModel">
                                                                    {provided => (
                                                                        ...
                                                                        >
                                                                            {
                                                                                selectSOP && selectSOP.sopModels.map((item, index) => {
                                                                                    if (item.deleted == 0) {
                                                                                        return (
                                                                                            <Draggable key={index} draggableId={`3DModel-index-${index}`} index={index}>
                                                                                                {provided => (
                                                                                                    <div
                                                                                                        className="col-sm-6 col-md-3 mb-2 d-flex justify-content-center"
                                                                                                        {...provided.draggableProps}
                                                                                                        ...
                                                                                                        ref={provided.innerRef}
                                                                                                    >
                                                                                                        {item.sopModelImage != "" || item.sopModelImageObj != null ?
                                                                                                            <div className="sop-model-col">
                                                                                                                <img src={item.sopModelImageObj != null ? URL.createObjectURL(item.sopModelImageObj) : item.sopModelImage} style={{ width: "100%", height: "100%" }} />
                                                                                                                <div
                                                                                                                    className="remove-temp-model"
                                                                                                                    style={{ position: "absolute", top: "0px", right: "7px" }}
                                                                                                                    onClick={() => handleCleanModelBtn(index)}
                                                                                                                >
                                                                                                                    <i className="fas fa-times"></i>
                                                                                                                </div>
                                                                                                            </div>
                                        ...
                                        <div className="d-flex justify-content-center align-items-center" style={{ height: '80vh' }}>
                                            <div><strong>{t("sop.emptySelectSOP")}{/*尚未選擇SOP*/}</strong></div>
                                        </div>
                                }
                            </div>
                        </div>
            ...
            <Modal show={showSaveTempModelModal} onHide={(e) => handleCloseSaveTempModelModal(e)} backdrop="static">
                    ...
                    <form>
                        <div className="row">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("sop.tempModelImage")}{/*3D模型圖片*/}</label>
                                <input
                                    type="file"
                                    className="form-control d-none"
                                    name="machineImage"
                                ...
                                >
                                    {
                                        selectTempModel && selectTempModel.tempModelImageObj != null ?
                                            <img
                                                alt="not found"
                                                style={{ width: "100px", height: "100px" }}
                                                src={selectTempModel.tempModelImageObj != null ? URL.createObjectURL(selectTempModel.tempModelImageObj) : null}
                                            /> :
                            ...
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("sop.tempModelFile")}{/*3D模型檔案*/}</label><br />
                                {(() => {
                                    var fileExtension = "";
                                    if (selectTempModel && selectTempModel.tempModelFileObj != null) {
                                        fileExtension = selectTempModel.tempModelFileObj.name.substr(selectTempModel.tempModelFileObj.name.lastIndexOf(".") + 1 - selectTempModel.tempModelFileObj.name.length).toLowerCase();
                                    }

                                    return fileExtension != "" ? <img src={`/${fileExtension}.png`} /> : null
                                })()}
                                <input
                                    type="file"
                                    className="form-control"
            ...
            <Modal show={showDeleteSOPModal} onHide={(e) => handleCloseDeleteSOPModal(e)} backdrop="static">
                <Modal.Header closeButton>
                    <Modal.Title>{t("sop.deleteSOP")}{/*刪除SOP*/}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <p>{t("sop.deleteContent")}{/*您確定要刪除該筆資料嗎?*/}</p>
                </Modal.Body>
                <Modal.Footer>
                    <button
                        type="button"
