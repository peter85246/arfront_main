...
function MachineAlarm() {
    ...
    const [machineAlarm, setMachineAlarm] = useState({ //單一Alarm
        machineAlarmId: 0,
        machineId: machineId,
        machineAlarmCode: "",
        machineAlarmAbstract: "",
    });
    const [machineAlarmErrors, setMachineAlarmErrors] = useState({ //機台Alarm錯誤訊息
        machineAlarmCode: "",
        machineAlarmAbstract: "",
    });
    ...
    const [showDeleteMachineAlarmModal, setShowDeleteMachineAlarmModal] = useState(false); //顯示"刪除IOT modal"
    const [saveDeleteMachineAlarmLoading, setSaveDeleteMachineAlarmLoading] = useState(false);
    ...
    const refreshMachineAlarms = async () => {
        ...
        if (machineAlarmResponse) {
            if (machineAlarmResponse.code == "0000") {
                ...
                setShowMachineAlarms(machineAlarmResponse.result.slice((activePage * pageRow) - pageRow, (activePage * pageRow)));
            }
        }
    }
    //#endregion
    ...
    let pages = []; //頁碼
    for (let number = 1; number <= Math.ceil(machineAlarms.length / pageRow); number++) {
        pages.push(
    ...
    const handleChangePage = async (e, number) => {
        ...
        setShowMachineAlarms(machineAlarms.slice((number * pageRow) - pageRow, (number * pageRow)));
    }
    //#endregion
    ...
    const handleOpenMachineAlarmModal = async (machineAlarmId) => {
        if (machineAlarmId == 0) {
            ...
            var tempMachineAlarm = machineAlarms.find(x => x.machineAlarmId == machineAlarmId);
    ...
    const checkEditValidator = async (name = "", val = "") => {
        ...
        if (name == "machineAlarmAbstract" || name == "") {
            ...
            else if (!validator.check(machineAlarm.machineAlarmAbstract, "max:1000")) {
    ...
    return (
        <>
                    ...
                    <div className="row mb-2 justify-content-between">
                        <div>
                            <a href={`/machine`} className="d-flex align-items-center">
                                <i className="fas fa-angle-left"></i>&nbsp;&nbsp;{t("machine.content.header")}{/*機台管理*/}
                            </a>
                        </div>
                        <div className="content-header-text-color">
                            <h1><strong>{t("machineAlarm.content.header")}{/*Alarm管理*/}</strong></h1>
                        </div>
                        <button type="button" className="btn btn-primary btn-add" onClick={() => handleOpenMachineAlarmModal(0)}>
                            <i className="fas fa-plus"></i> {t("machineAlarm.btn.add")}{/*新增Alarm*/}</button>
                    </div>
                </div>
            </section>
                    ...
                    <div className="card">
                        <div className="card-body table-responsive p-0">
                            <table className="table table-striped table-valign-middle">
                                <thead>
                                    <tr>
                                        <th>{t("machineAlarm.MachineAlarmId")}{/*編號*/}</th>
                                        <th>{t("machineAlarm.MachineAlarmCode")}{/*故障代碼*/}</th>
                                        <th>{t("machineAlarm.MachineAlarmAbstract")}{/*故障說明*/}</th>
                                        <th>{t("machineAlarm.fun")}{/*功能*/}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {showMachineAlarms && showMachineAlarms.length > 0 ?
                                            ...
                                            {showMachineAlarms.map((item, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            <button type="button" className="btn btn-outline-primary" onClick={() => handleOpenMachineAlarmModal(item.machineAlarmId)}>{t("machineAlarm.btn.edit")}{/*編輯*/}</button>{" "}
                                                            <button type="button" className="btn btn-outline-info" onClick={() => handleGotoSOP(item.machineAlarmId)}>{t("machineAlarm.btn.sop")}{/*SOP*/}</button>{" "}
                                                            <button type="button" className="btn btn-outline-danger" onClick={() => handleOpenDeleteMachineAlarmModal(item.machineAlarmId)}>{t("machineAlarm.btn.del")}{/*刪除*/}</button>{" "}
                                                            <button type="button" className="btn btn-outline-secondary">{t("machineAlarm.btn.pdf")}{/*PDF*/}</button>
                                                        </td>
                                                    </tr>
                                                )
                                            })}
                                        ...
                                        <>
                                            <tr>
                                                <td colSpan="4" style={{ textAlign: 'center' }}>{t("table.empty")}{/*查無資料*/}</td>
                                            </tr>
                                        </>
                                    }
                                </tbody>
            ...
            <Modal show={showMachineAlarmModal} onHide={(e) => handleCloseMachineAlarmModal(e)} backdrop="static">
                <Modal.Header closeButton>
                    <Modal.Title>{machineAlarm.machineAlarmId == 0 ? t("machineAlarm.add") : t("machineAlarm.edit")}{/*新增Alarm : 編輯Alarm*/}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <form>
                        {machineAlarm.machineAlarmId != 0 ?
                            <div className="row mb-3">
                                <div className="col-12 form-group">
                                    <label className="form-label">{t("machineAlarm.MachineAlarmId")}{/*編號*/}</label>
                                    <span className="form-text">{machineAlarm.machineAlarmId}</span>
                                </div>
                            </div>
                            : <></>
                        }
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("machineAlarm.MachineAlarmCode")}{/*故障代碼*/}</label>
                                <input
                                    type="text"
                        ...
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("machineAlarm.MachineAlarmAbstract")}{/*故障說明*/}</label>
                                <input
                                    type="text"
                                    className="form-control"
                                ...
                                {(() => {
                                    switch (machineAlarmErrors.machineAlarmAbstract) {
                                        case 'required':
                                            return <div className="invalid-feedback d-block"><i className="fas fa-exclamation-circle"></i> {t("helpWord.required")} {/*不得空白*/}</div>
                                        case 'max':
                                            return <div className="invalid-feedback d-block"><i className="fas fa-exclamation-circle"></i> {t("helpWord.max", { e: 1000 })}{/*超過上限{{e}}個字元*/}</div>
                                        default:
                                            return null
                                    }
                                })()}
            ...
            <Modal show={showDeleteMachineAlarmModal} onHide={(e) => handleCloseDeleteMachineAlarmModal(e)} backdrop="static">
                <Modal.Header closeButton>
                    <Modal.Title>{t("machineAlarm.delete")}{/*刪除Alarm*/}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <p>{t("machineAlarm.deleteContent")}{/*您確定要刪除該筆資料嗎?*/}</p>
                </Modal.Body>
                <Modal.Footer>
                    <button
                        type="button"
