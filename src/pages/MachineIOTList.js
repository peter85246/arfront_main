...
function MachineIOTList() {
    ...
    const [showDeleteMachineIOTModal, setShowDeleteMachineIOTModal] = useState(false); //顯示"刪除IOT modal"
    const [saveDeleteMachineIOTLoading, setSaveDeleteMachineIOTLoading] = useState(false);
    ...
    const refreshMachineIOTs = async () => {
        ...
        if (iotOverviewResponse) {
            if (iotOverviewResponse.code == "0000") {
                ...
                setShowMachineIOTs(iotOverviewResponse.result.slice((activePage * pageRow) - pageRow, (activePage * pageRow)));
            }
        }
    }
    //#endregion
    ...
    let pages = []; //頁碼
    for (let number = 1; number <= Math.ceil(machineIOTs.length / pageRow); number++) {
        pages.push(
    ...
    const handleChangePage = async (e, number) => {
        ...
        setShowMachineIOTs(machineIOTs.slice((number * pageRow) - pageRow, (number * pageRow)));
    }
    //#endregion
    ...
    return (
        <>
                ...
                <div className="container-fluid">
                    <div className="row mb-2 justify-content-between align-items-center">
                        ...
                        <div className="content-header-text-color">
                            <h1><strong>{t("machineIOTList.content.header")}{/*IOT管理*/}</strong></h1>
                        </div>
                        <button type="button" className="btn btn-primary btn-add" onClick={() => handleGotoMachineIOT(0)}>
                            <i className="fas fa-plus"></i> {t("machineIOTList.btn.add")}{/*新增IOT*/}</button>
                    </div>
                </div>
            </section>
                    ...
                    <div className="card">
                        <div className="card-body table-responsive p-0">
                            <table className="table table-striped table-valign-middle">
                                <thead>
                                    <tr>
                                        <th>{t("machineIOTList.machineIOTId")}{/*編號*/}</th>
                                        <th>{t("machineIOT.machineIOTDeviceName")}{/*設備名稱*/}</th>
                                        <th>{t("machineIOT.machineIOTMQTTBroker")}{/*Server*/}</th>
                                        <th>{t("machineIOT.machineIOTClientId")}{/*Client ID*/}</th>
                                        <th>{t("machineIOTList.fun")}{/*功能*/}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                            ...
                                            {showMachineIOTs.map((item, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            <button type="button" className="btn btn-outline-primary" onClick={() => handleGotoMachineIOT(item.machineIOTId)}>{t("machineIOTList.btn.edit")}{/*編輯*/}</button>{" "}
                                                            <button type="button" className="btn btn-outline-danger" onClick={() => handleOpenDeleteMachineIOTModal(item.machineIOTId)}>{t("machineIOTList.btn.del")}{/*刪除*/}</button>
                                                        </td>
                                                    </tr>
                                                )
                                            })}
            ...
            <Modal show={showDeleteMachineIOTModal} onHide={(e) => handleCloseDeleteMachineIOTModal(e)} backdrop="static">
                <Modal.Header closeButton>
                    <Modal.Title>{t("machineIOTList.delete")}{/*刪除IOT*/}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <p>{t("machineIOTList.deleteContent")}{/*您確定要刪除該筆資料嗎?*/}</p>
                </Modal.Body>
                <Modal.Footer>
                    <button
                        type="button"
