...
function UserManage() {
    ...
    const [addUserinfo, setAddUserinfo] = useState({ //新增單一使用者
        userId: 0,
        userName: "",
        userAccount: "",
        userPaw: "",
    ...
    const [addUserinfoErrors, setAddUserinfoErrors] = useState({ //新增單一使用者錯誤訊息
        userName: "",
        userAccount: "",
        userPaw: "",
        userAgainPaw: "",
    ...
    const [editUserinfo, setEditUserinfo] = useState({ //修改單一使用者
        userId: 0,
        userName: "",
        userAccount: "",
        userLevel: 0,
    });
    const [editUserinfoErrors, setEditUserinfoErrors] = useState({ //修改單一使用者錯誤訊息
        userName: "",
        userLevel: "",
    });
    ...
    const [saveDeleteUserinfoLoading, setSaveDeleteUserinfoLoading] = useState(false);

    const [selectChangePawId, setSelectChangePawId] = useState(0); //要修改密碼的使用者id
    const [showUserinfoChangePawModal, setShowUserinfoChangePawModal] = useState(false); //顯示"修改使用者密碼modal"
    const [saveUserinfoChangePawLoading, setSaveUserinfoChangePawLoading] = useState(false);
    const [userinfoChangePawErrors, setUserinfoChangePawErrors] = useState({ //修改密碼錯誤訊息
        newPaw: "",
        againPaw: "",
    });
        ...
        validators: {
            pawFormat: {
                rule: (val, params, validator) => {
                    ...
                    strRep = reactStringReplace(strRep, /([,.~!@#$%^&*_+-=])/g, (match, i) => (""));
    ...
    const refreshUserinfos = async () => {
        ...
        if (userinfosResponse) {
            if (userinfosResponse.code == "0000") {
                ...
                setShowUserinfos(userinfosResponse.result.slice((activePage * pageRow) - pageRow, (activePage * pageRow)));
            }
        }
    }
    //#endregion
    ...
    let pages = []; //頁碼
    for (let number = 1; number <= Math.ceil(userinfos.length / pageRow); number++) {
        pages.push(
    ...
    const handleChangePage = async (e, number) => {
        ...
        setShowUserinfos(userinfos.slice((number * pageRow) - pageRow, (number * pageRow)));
    }
    //#endregion
    ...
    const checkAddValidator = async (name = "", val = "") => {
        ...
        if (name == "userPaw" || name == "") {
            ...
            else if (tempPaw != tempAgainPaw && addUserinfoErrors.userAgainPaw == "") {
    ...
    const checkChangePawValidator = async (name = "", val = "") => {
        ...
        if (name == "newPaw" || name == "") {
            ...
            else if (tempNewPaw != tempAgainPaw && userinfoChangePawErrors.againPaw == "") {
    ...
    return (
        <>
                ...
                <div className="container-fluid">
                    <div className="row mb-2 justify-content-between">
                        ...
                        <div className="content-header-text-color">
                            <h1><strong>{t("userManage.content.header")}{/*使用者管理*/}</strong></h1>
                        </div>
                        <button type="button" className="btn btn-primary btn-add" onClick={(e) => handleOpenAddUserinfoModal(e)}>
                            <i className="fas fa-plus"></i> {t("userManage.btn.add")}{/*新增使用者*/}</button>
                    </div>
                </div>
            </section>
                    ...
                    <div className="card">
                        <div className="card-body table-responsive p-0">
                            <table className="table table-striped table-valign-middle">
                                <thead>
                                    <tr>
                                        <th>{t("userManage.userId")}{/*編號*/}</th>
                                        <th>{t("userManage.userName")}{/*名稱*/}</th>
                                        <th>{t("userManage.userAccount")}{/*帳號*/}</th>
                                        <th>{t("userManage.userLevel")}{/*層級*/}</th>
                                        <th>{t("userManage.fun")}{/*功能*/}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {showUserinfos && showUserinfos.length > 0 ?
                                        <>
                                            {showUserinfos.map((item, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            <button type="button" className="btn btn-outline-primary" onClick={() => handleOpenEditUserinfoModal(item.userId)}>{t("userManage.btn.edit")}{/*編輯*/}</button>{" "}
                                                            {
                                                                myUser && myUser.userId != item.userId ?
                                                                    <>
                                                                        <button type="button" className="btn btn-outline-info" onClick={() => handleOpenUserinfoChangePawModal(item.userId)}> {t("userManage.changePaw")}{/*修改密碼*/}</button>{" "}
                                                                    </>
                                                                    :
                                                                    <></>
                                                            }
                                                            {
                                                                myUser && myUser.userId != item.userId ?
                                                                    <button type="button" className="btn btn-outline-danger" onClick={() => handleOpenDeleteUserinfoModal(item.userId)}>{t("userManage.btn.del")}{/*刪除*/}</button>
                                                                    :
                                                                    <></>
                                                            }
                                                        </td>
                                            ...
                                            <tr>
                                                <td colSpan="5" style={{ textAlign: 'center' }}>{t("table.empty")}{/*查無資料*/}</td>
                                            </tr>
                                        </>
                                    }
                                </tbody>
            ...
            <Modal show={showAddUserinfoModal} onHide={(e) => handleCloseAddUserinfoModal(e)} backdrop="static">
                <Modal.Header closeButton>
                    <Modal.Title>{t("userManage.add")}{/*新增使用者*/}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <form>
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("userManage.userName")}{/*名稱*/}</label>
                                <input
                                    type="text"
                                ...
                                {(() => {
                                    switch (addUserinfoErrors.userName) {
                                        ...
                                        case 'max':
                                            return <div className="invalid-feedback d-block"><i className="fas fa-exclamation-circle"></i> {t("helpWord.max", { e: 50 })}{/*超過上限{{e}}個字元*/}</div>
                                        default:
                                            return null
                                    }
                                })()}
                        ...
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("userManage.userAccount")}{/*帳號*/}</label>
                                <input
                                    type="text"
                                    className="form-control"
                                    name="userAccount"
                        ...
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("userManage.userPaw")}{/*密碼*/}</label>
                                <input
                                    type="password"
                                    className="form-control"
                                    name="userPaw"
                                ...
                                {(() => {
                                    switch (addUserinfoErrors.userPaw) {
                                        case 'required':
                                            return <div className="invalid-feedback d-block"><i className="fas fa-exclamation-circle"></i> {t("helpWord.required")} {/*不得空白*/}</div>
                                        case 'pawFormat':
                                            return <div className="invalid-feedback d-block"><i className="fas fa-exclamation-circle"></i> {t("helpWord.format")} {/*格式有誤*/}</div>
                                        default:
                                            return null
                                    }
                                })()}
                        ...
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("userManage.userAgainPaw")}{/*再次輸入密碼*/}</label>
                                <input
                                    type="password"
                                    className="form-control"
                                    name="userAgainPaw"
                                ...
                                {(() => {
                                    switch (addUserinfoErrors.userAgainPaw) {
                                        ...
                                        case 'pawFormat':
                                            return <div className="invalid-feedback d-block"><i className="fas fa-exclamation-circle"></i> {t("helpWord.format")} {/*格式有誤*/}</div>
                                        case 'again':
                                            return <div className="invalid-feedback d-block"><i className="fas fa-exclamation-circle"></i> {t("helpWord.pwdIn")} {/*兩次密碼輸入不相同*/}</div>
                                        default:
                                            return null
                                    }
                                })()}
                        ...
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("userManage.userLevel")}{/*層級*/}</label>
                                <div>
                                    <div className="form-check-inline">
                                        <div className="custom-control custom-radio">
                                            <input
                                                ...
                                                onChange={(e) => handleAddChange(e)}
                                            />
                                            <label htmlFor="user_level_0" className="custom-control-label">{t("userManage.userLevel.admin")}{/*最高管理者*/}</label>
                                        </div>
                                    </div>
                                    <div className="form-check-inline">
                                        <div className="custom-control custom-radio form-check-inline">
                                            <input
                                                ...
                                                onChange={(e) => handleAddChange(e)}
                                            />
                                            <label htmlFor="user_level_1" className="custom-control-label">{t("userManage.userLevel.expert")}{/*專家*/}</label>
                                        </div>
                                    </div>
                                    <div className="form-check-inline">
                                        <div className="custom-control custom-radio form-check-inline">
                                            <input
                                                ...
                                                onChange={(e) => handleAddChange(e)}
                                            />
                                            <label htmlFor="user_level_2" className="custom-control-label">{t("userManage.userLevel.user")}{/*一般用戶*/}</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <Modal show={showEditUserinfoModal} onHide={(e) => handleCloseEditUserinfoModal(e)} backdrop="static">
                <Modal.Header closeButton>
                    <Modal.Title>{t("userManage.edit")}{/*編輯使用者*/}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <form>
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label">{t("userManage.userId")}{/*編號*/}</label>
                        ...
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label">{t("userManage.userAccount")}{/*帳號*/}</label>
                                <input
                                    type="text"
                                    className="form-control"
                                    name="userAccount"
            ...
            <Modal show={showDeleteUserinfoModal} onHide={(e) => handleCloseDeleteUserinfoModal(e)} backdrop="static">
                <Modal.Header closeButton>
                    <Modal.Title>{t("userManage.delete")}{/*刪除使用者*/}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <p>{t("userManage.deleteContent")}{/*您確定要刪除該筆資料嗎?*/}</p>
                </Modal.Body>
                <Modal.Footer>
                    <button
                        type="button"
            ...
            <Modal show={showUserinfoChangePawModal} onHide={(e) => handleCloseUserinfoChangePawModal(e)} backdrop="static">
                <Modal.Header closeButton>
                    <Modal.Title>{t("header.changePaw")}{/*修改密碼*/}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <form>
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("header.newPaw")}{/*新密碼*/}</label>
                                <input
                                    type="password"
                                    className="form-control"
                                    name="newPaw"
                        ...
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("header.againPaw")}{/*再次輸入密碼*/}</label>
                                <input
                                    type="password"
                                    className="form-control"
                                    name="againPaw"
