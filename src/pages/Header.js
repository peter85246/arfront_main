...
import { hasWindowClass, addWindowClass, removeWindowClass } from '../utils/helpers';
...
function Header() {
    ...
    const [changePawErrors, setChangePawErrors] = useState({ //修改密碼錯誤訊息
        oldPaw: "",
        newPaw: "",
        againPaw: "",
    });
    ...
    return (
        <>
            <nav className="main-header navbar navbar-expand navbar-light">
                <ul className="navbar-nav">
                    <li className="nav-item">
                        <button type="button" className="nav-link" onClick={(e) => handleToggleNav(e)}>
                ...
                <ul className="navbar-nav ml-auto" ref={wrapperRef}>
                    <li className={`nav-item dropdown ${toggleUser ? "show" : ""}`}>
                        <a href={void (0)} className="nav-link" onClick={(e) => handleToggleUserDropdown(e)}>
                            {t("header.user", { e: myUser ? myUser.userName : "" })}&nbsp;
                            <i className="fas fa-user"></i>
                        </a>
                        <div className={`dropdown-menu dropdown-menu-right ${toggleUser ? "show" : ""}`}>
                            <a href={void (0)} className="dropdown-item" onClick={(e) => handleOpenChangePawModal(e)}>
                                <i className="fas fa-lock"></i> {t("header.changePaw")}{/*修改密碼*/}
                            </a>
                            <a href={void (0)} className="dropdown-item" onClick={(e) => handleSignOut(e)}>
                                <i className="fas fa-sign-out-alt"></i> {t("header.logout")}{/*登出*/}
                            </a>
                        </div>
                    </li>
                </ul>
            ...
            <Modal show={showChangePawModal} onHide={(e) => handleCloseChangePawModal(e)} backdrop="static">
                    ...
                    <form>
                        <div className="row mb-3">
                            <div className="col-12 form-group">
                                <label className="form-label"><span className="text-danger">*</span>{t("header.oldPaw")}{/*舊密碼*/}</label>
                                <input
                                    type="password"
